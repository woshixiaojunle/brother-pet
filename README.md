...
| 🖼️ 直接支持 GIF 动图 | 每个动作可选 `.gif` 素材，运行时按原生帧时长逐帧播放（透明通道保留，等比适配画布） |
...
|------|------|------|
| GET  | `/api/health`        | 健康检查 |
| GET  | `/api/samples`       | 返回可用示例素材列表（含 `assets/` 与 `generated-gifs/`，供前端下拉选择） |
| GET  | `/api/sample/{filename}` | 返回指定示例素材文件本体（前端预览用，支持 GIF 动画预览） |
...
1. 添加 / 删除宠物，选择示例素材或上传素材（**PNG 静态帧** 或 **GIF 动图**，透明背景最佳）
2. 每个动作（爬行 / 攀爬 / 发呆 / 开心）都可单独指定一张 PNG 或 GIF；选 GIF 时右侧会实时预览动画
3. 调整爬行速度、跳跃频率、发呆频率
4. 填写「叫爸爸」台词池与「投喂」文本
5. 指定输出 exe 路径（如 `D:\brother-pet\BrotherPet.exe`）
6. 点击「生成 exe」，等待进度条完成，下载即可双击运行

> 💡 `generated-gifs/` 目录已内置 AI 生成的 61 帧 GIF（爬行 / 攀爬 / 开心 / 跳跃），可直接作为示例素材选用，运行时将按原生节奏逐帧播放。
